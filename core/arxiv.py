...
def _parse_entry(entry: ET.Element) -> dict:
    ...
    title = " ".join(title.split())  # strip whitespace and newlines
    ...
    primary_cat_el = entry.find("arxiv:primary_category", namespaces=NAMESPACES)
    categories = primary_cat_el.attrib.get("term", "") if primary_cat_el is not None else ""
...
def search_papers(
    ...
    for entry in entries:
        ...
        # Skip entries that have no real ID (arXiv returns a placeholder entry on empty results)
